    ...
    * 
	...
	public boolean diagonalCheck(CellValue[][] current_board, int i){
		...
		if ((this.lines >= this.sizeWin) && (this.columns >= this.sizeWin)){
			...
			// Checking diagonally from bottom left to top right.
			// Starting from the top right and going down, each time
			...
			// Checking diagonally from bottom left to top right.
			// Starting from the bottom right (skipping the first 
			// one as it has already been checked) and going left, 
			// each time starting from the last cell of each column
